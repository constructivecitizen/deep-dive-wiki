...
interface HybridNavigationSidebarProps {
  ...
  return (
    <>
      ...
      <div
        className={`relative flex items-center gap-1.5 py-1 px-2 rounded-md group transition-colors cursor-pointer min-w-0 ${
          isActiveNode 
      ...
      <div className="p-2 border-t border-border bg-muted/50">
        <div className="flex items-center justify-center gap-2">
          ...
          >
            ...
            <SelectContent className="bg-popover z-50" align="center">
              {expandMode === 'mixed' && (
                <SelectItem value="mixed" disabled>-</SelectItem>
              )}
              {Array.from({ length: 21 }, (_, i) => (
                <SelectItem key={i} value={i.toString()} className="justify-center text-center">
                  {i}
                </SelectItem>
              ))}
            </SelectContent>
